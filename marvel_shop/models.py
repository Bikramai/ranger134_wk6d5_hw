...
class Product(db.Model): # db.Models helps us translate python code to columns in SQL
    ...
    def set_image(self, image, name):
        ...
        if not image: #aka the user did not give us an image
            image = Marvel(name) #name is going to become search & we are going to use it to make our api call 
...
class Order(db.Model):
    ...
    date_created = date_created = db.Column(db.DateTime, default = datetime.utcnow())
...
# data needs to look like a dictionary not an object)
